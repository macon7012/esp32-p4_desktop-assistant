...
esp_err_t lvgl_port_init(const lvgl_port_cfg_t *cfg)
{
    ...
    ESP_GOTO_ON_FALSE(cfg, ESP_ERR_INVALID_ARG, err, LVGL_TAG, "invalid argument");
    ESP_GOTO_ON_FALSE(cfg->task_affinity < (configNUM_CORES), ESP_ERR_INVALID_ARG, err, LVGL_TAG, "Bad core number for task! Maximum core number is %d", (configNUM_CORES - 1));
    ...
    ESP_RETURN_ON_ERROR(lvgl_port_tick_init(), LVGL_TAG, "");
    /* 创建任务 */
    ...
    ESP_GOTO_ON_FALSE(lvgl_port_ctx.lvgl_mux, ESP_ERR_NO_MEM, err, LVGL_TAG, "Create LVGL mutex fail!");
    /* 创建任务互斥锁 */
    lvgl_port_ctx.task_mux = xSemaphoreCreateMutex();
    ESP_GOTO_ON_FALSE(lvgl_port_ctx.task_mux, ESP_ERR_NO_MEM, err, LVGL_TAG, "Create LVGL task sem fail!");
    ...
    ESP_GOTO_ON_FALSE(res == pdPASS, ESP_FAIL, err, LVGL_TAG, "Create LVGL task fail!");
...
esp_err_t lvgl_port_deinit(void)
{
    ...
    {
        ESP_LOGE(LVGL_TAG, "Failed to stop LVGL task");
        return ESP_ERR_TIMEOUT;
    }
    ESP_LOGI(LVGL_TAG, "Stopped LVGL task");
...
{
    ESP_LOGE(LVGL_TAG, "Task wake is not supported, when used LVGL8!");
...
static void lvgl_port_task(void *arg)
{
    ...
    {
        ESP_LOGE(LVGL_TAG, "Failed to take LVGL task sem");
        lvgl_port_task_deinit();
        vTaskDelete(NULL);
    }
...
static esp_err_t lvgl_port_tick_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(esp_timer_create(&lvgl_tick_timer_args, &lvgl_port_ctx.tick_timer), LVGL_TAG, "Failed to take LVGL task timer");
