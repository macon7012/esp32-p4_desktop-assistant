...
static lv_disp_t *lvgl_port_add_disp_priv(const lvgl_port_display_cfg_t *disp_cfg, const lvgl_port_disp_priv_cfg_t *priv_cfg)
{
    ...
    ESP_GOTO_ON_FALSE(disp_ctx, ESP_ERR_NO_MEM, err, LVGL_TAG, "Not enough memory for display context allocation!"); /* 内存分配失败则跳转到错误处理 */
    ...
    if (priv_cfg && priv_cfg->avoid_tearing)
    {
        ...
        {
            ESP_GOTO_ON_ERROR(esp_lcd_dpi_panel_get_frame_buffer(disp_cfg->panel_handle, 2, (void *)&buf1, (void *)&buf2), err, LVGL_TAG, "Get RGB buffers failed"); /* 获取RGB缓冲区 */
        }
        else /* 如果是RGB屏幕 */
        {
            ESP_GOTO_ON_ERROR(esp_lcd_rgb_panel_get_frame_buffer(disp_cfg->panel_handle, 2, (void *)&buf1, (void *)&buf2), err, LVGL_TAG, "Get RGB buffers failed"); /* 获取RGB缓冲区 */
        }
#endif
        trans_sem = xSemaphoreCreateCounting(1, 0);                                                                   /* 创建计数信号量 */
        ESP_GOTO_ON_FALSE(trans_sem, ESP_ERR_NO_MEM, err, LVGL_TAG, "Failed to create transport counting Semaphore"); /* 信号量创建失败则跳转到错误处理 */
    ...
    else
    {
        ...
        {
            ESP_GOTO_ON_FALSE(false, ESP_ERR_NOT_SUPPORTED, err, LVGL_TAG, "Alloc DMA capable buffer in SPIRAM is not supported!"); /* 不支持在SPIRAM中分配DMA缓冲区 */
        }
        ...
        if (disp_cfg->trans_size) /* 如果需要传输缓冲区 */
        {
            ...
            ESP_GOTO_ON_FALSE(buf3, ESP_ERR_NO_MEM, err, LVGL_TAG, "Not enough memory for buffer(transport) allocation!"); /* 缓冲区分配失败则跳转到错误处理 */
        ...
        ESP_GOTO_ON_FALSE(buf1, ESP_ERR_NO_MEM, err, LVGL_TAG, "Not enough memory for LVGL buffer (buf1) allocation!"); /* 缓冲区分配失败则跳转到错误处理 */

        if (disp_cfg->double_buffer) /* 如果需要双缓冲 */
        {
            buf2 = heap_caps_malloc(buffer_size * sizeof(lv_color_t), buff_caps);                                           /* 分配第二个缓冲区 */
            ESP_GOTO_ON_FALSE(buf2, ESP_ERR_NO_MEM, err, LVGL_TAG, "Not enough memory for LVGL buffer (buf2) allocation!"); /* 缓冲区分配失败则跳转到错误处理 */
        }
    }

    /* 分配LVGL显示缓冲区 */
    lv_disp_draw_buf_t *disp_buf = malloc(sizeof(lv_disp_draw_buf_t));
    ESP_GOTO_ON_FALSE(disp_buf, ESP_ERR_NO_MEM, err, LVGL_TAG, "Not enough memory for LVGL display buffer allocation!"); /* 缓冲区分配失败则跳转到错误处理 */

    /* 初始化LVGL绘图缓冲区 */
    lv_disp_draw_buf_init(disp_buf, buf1, buf2, buffer_size);
    ESP_LOGD(LVGL_TAG, "Register display driver to LVGL");  /* 注册显示驱动到LVGL */
    ...
    {
        /* 单色显示必须使用全缓冲区 */
        ESP_GOTO_ON_FALSE((disp_cfg->hres * disp_cfg->vres == buffer_size), ESP_ERR_INVALID_ARG, err, LVGL_TAG, "Monochromatic display must using full buffer!");
    ...
    {
        /* 直接模式必须使用全缓冲区 */
        ESP_GOTO_ON_FALSE((disp_cfg->hres * disp_cfg->vres == buffer_size), ESP_ERR_INVALID_ARG, err, LVGL_TAG, "Direct mode must using full buffer!");
    ...
    {
        /* 全刷新模式必须使用全缓冲区 */
        ESP_GOTO_ON_FALSE((disp_cfg->hres * disp_cfg->vres == buffer_size), ESP_ERR_INVALID_ARG, err, LVGL_TAG, "Full refresh must using full buffer!");
