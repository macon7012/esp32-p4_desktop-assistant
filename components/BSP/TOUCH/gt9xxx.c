...
esp_err_t gt9xxx_init(void)
{
    ...
    /* I2C总线上添加gt9xxx设备 */
    ESP_ERROR_CHECK(i2c_master_bus_add_device(my_bus_handle, &gt9xxx_i2c_dev_conf, &gt9xxx_handle));
        ...
        if ((mode & 0XF) && ((mode & 0XF) <= g_gt_tnum))
        {
            ...
            for (i = 0; i < g_gt_tnum; i++)
            {
                if (tp_dev.sta & (1 << i))                      /* 触摸有效? */
                {
					...
					if (lcddev.id == 0x9881d || lcddev.id == 0x7703 || lcddev.id == 0x9881c8 || lcddev.id == 0x9881c10 || mipidev.id == 0x79007)        /* 5/5.5/8/10.1寸MIPI屏触摸屏 */
					{
